...
import sys, os, mpi4pyve

MPI4PYPATH = os.path.abspath(os.path.dirname(mpi4pyve.__path__[0]))
...
def childscript():
    ...
    fd, script = mkstemp(suffix='.py', prefix="mpi4pyve-")
    os.close(fd)
    with open(script, "w") as f:
        ...
        import mpi4pyve
        if %(mpe)s: mpi4pyve.profile('mpe', logfile="%(logfile)s")
        if %(vt)s:  mpi4pyve.profile('vt',  logfile="%(logfile)s")
        from mpi4pyve import MPI
        ...
        """ % dict(python=sys.executable, path=MPI4PYPATH,
                   ...
                   logfile="runtests-mpi4pyve-child")))
