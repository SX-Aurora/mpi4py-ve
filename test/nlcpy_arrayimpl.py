...
if nlcpy is not None:
    ...
    class NlcPy(object):
        ...
        def __init__(self, arg, typecode, shape=None):
            ...
            self.array = ary = nlcpy.zeros(shape, typecode)
            if isinstance(arg, (int, float, complex)):
                ary.fill(arg)
            else:
                ary[:] = arg
            try:
                self.mpidtype = NlcPy.TypeMap[typecode]
            except KeyError:
        ...
        def flat(self):
            ...
            f = self.array.reshape(self.array.size)
            self.array.reshape(_shape)
            return f
        flat = property(flat)
    ...
    ArrayTypes.append(NlcPy)
    __all__.append('NlcPy')
