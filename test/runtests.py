...
def import_package(options, pkgname):
    ...
    if options.threads is not None:
        mpi4pyve.rc.threads = options.threads
    if options.thread_level is not None:
        mpi4pyve.rc.thread_level = options.thread_level
    if options.mpe:
        mpi4pyve.profile('mpe', logfile='runtests-mpi4pyve')
    if options.vt:
        mpi4pyve.profile('vt', logfile='runtests-mpi4pyve')
    import mpi4pyve.MPI
    #
    return package
