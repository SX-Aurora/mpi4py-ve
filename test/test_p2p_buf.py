...
import nlcpy_arrayimpl as arrayimpl
...
class BaseTestP2PBuf(object):
        ...
        for array in arrayimpl.ArrayTypes:
            for typecode in arrayimpl.TypeMap:
                for s in range(0, size):
                    ...
                    rbuf = array(-1, typecode, s)
                    rreq = self.COMM.Irecv(rbuf.as_mpi(), rank, 0)
                    self.COMM.Isend(sbuf.as_mpi(), rank, 0).Wait()
                    rreq.Wait()
                    for value in rbuf:
                        self.assertEqual(value, s)

    def testISendIRecv_Test(self):
        size = self.COMM.Get_size()
        rank = self.COMM.Get_rank()
        for array in arrayimpl.ArrayTypes:
            for typecode in arrayimpl.TypeMap:
    ...
    def testISendIRecv_Waitall(self):
        size = self.COMM.Get_size()
        rank = self.COMM.Get_rank()
        for array in arrayimpl.ArrayTypes:
            for typecode in arrayimpl.TypeMap:
                for s in range(0, size):
                    ...
                    sbufs = [array( s*n, typecode, s) for n in range(3)]
                    rbufs = [array(-1, typecode, s) for n in range(3)]
                    rreqs = [self.COMM.Irecv(rbuf.as_mpi(), rank, 0) for rbuf in rbufs]
                    for sbuf in sbufs:
                        self.COMM.Isend(sbuf.as_mpi(), rank, 0).Wait()
                    MPI.Request.Waitall(rreqs)
                    for n, rbuf in enumerate(rbufs):
                        for value in rbuf:
                            self.assertEqual(value, s*n)
                    

    def testISendIRecv_Testall(self):
        size = self.COMM.Get_size()
        rank = self.COMM.Get_rank()
        for array in arrayimpl.ArrayTypes:
            for typecode in arrayimpl.TypeMap:
    ...
    def testISendIRecv_Waitany(self):
        size = self.COMM.Get_size()
        rank = self.COMM.Get_rank()
        for array in arrayimpl.ArrayTypes:
            for typecode in arrayimpl.TypeMap:
                for s in range(0, size):
                    ...
                    rreq = self.COMM.Irecv(rbuf.as_mpi(), rank, 0)
                    self.COMM.Isend(sbuf.as_mpi(), rank, 0).Wait()
                    MPI.Request.Waitany([rreq])
                    for value in rbuf:
                        self.assertEqual(value, s)

                    sbufs = [array( s, typecode, s) for n in range(3)]
                    rbufs = [array(-1, typecode, s) for n in range(3)]
                    rreqs = [self.COMM.Irecv(rbuf.as_mpi(), rank, 0) for rbuf in rbufs] 
                    for sbuf in sbufs:
                        self.COMM.Isend(sbuf.as_mpi(), rank, 0).Wait()
    ...
    def testISendIRecv_Testany(self):
        size = self.COMM.Get_size()
        rank = self.COMM.Get_rank()
        for array in arrayimpl.ArrayTypes:
            for typecode in arrayimpl.TypeMap:
    ...
    def testISendIRecv_Waitsome(self):
        size = self.COMM.Get_size()
        rank = self.COMM.Get_rank()
        for array in arrayimpl.ArrayTypes:
            for typecode in arrayimpl.TypeMap:
                for s in range(0, size):
                    ...
                    for sbuf in sbufs:
                        self.COMM.Isend(sbuf.as_mpi(), rank, 0).Wait()
                    ret = MPI.Request.Waitsome(rreqs)
                    for n, rbuf in enumerate(rbufs):
                        if n in ret:
                            for value in rbuf:
                                self.assertEqual(value, s)

    def testISendIRecv_Testsome(self):
        size = self.COMM.Get_size()
        rank = self.COMM.Get_rank()
        for array in arrayimpl.ArrayTypes:
            for typecode in arrayimpl.TypeMap:
                for s in range(0, size):
                    ...
                    for sbuf in sbufs:
                        self.COMM.Isend(sbuf.as_mpi(), rank, 0).Wait()
                    ret = MPI.Request.Testsome(rreqs)
                    for n, rbuf in enumerate(rbufs):
                        if n in ret:
                            for value in rbuf:
                                self.assertEqual(value, s)
