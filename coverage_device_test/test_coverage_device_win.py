...
class TestCoverageDeviceWin(unittest.TestCase):
    ...
    def test_Compare_and_swap(self, dev1, dev2, shape, dtype, order):
        ...
        x.fill(1)
        y.fill(0)
        z.fill(dev2.array(-1).astype(z.dtype))
        self.WIN.Fence()
        ...
        actual_x.fill(1)
        actual_y.fill(0)
        actual_z.fill(np.array(-1).astype(z.dtype))
        self.actual_WIN.Fence()
